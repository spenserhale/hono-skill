---
name: hono
description: Comprehensive offline reference for the Hono web framework ecosystem — the full docs (84 pages covering routing, middleware, helpers, RPC, JSX, validation, testing, and every runtime target), plus the starter template, the create-hono scaffolder, and the official example apps. Invoke this skill whenever the user is building, debugging, explaining, or porting anything in Hono — routes, middleware (CORS, JWT, bearer/basic auth, logger, cache, compress, ETag, CSRF, rate-limiting via timeout, etc.), helpers (cookies, JWT, SSG, streaming, WebSocket), deployment to Cloudflare Workers / Pages, Bun, Deno, Node, Vercel, Netlify, AWS Lambda, Lambda@Edge, Fastly, Azure Functions, Google Cloud Run, Supabase Functions, or Ali Function Compute, the RPC client `hc`, JSX/SSR, or validation via Zod/Valibot/ArkType. Use this skill even if the user doesn't say "Hono" by name — if they import from `hono` or `@hono/*`, or their project includes `hono` in package.json, this skill is relevant. For CLI-related tasks (`hono docs`, `hono serve`, `hono request`, `hono optimize`), defer to the separate `hono-cli` skill.
---
